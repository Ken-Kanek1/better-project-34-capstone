...
function preload() {
  ...
  playerimage = loadImage("./assets/dabi.png");
}
...
function setup() {
  ...
  player = Bodies.rectangle(250, playerBase.position.y - 160, 100, 100, options);
  ...
  board2 = new Board(width - 550, height - 300, 100, 200);
}

function draw() {
  background(backgroundImg );
  image(baseimage,playerBase.position.x,playerBase.position.y,180,150);
  image(playerimage,player.position.x,player.position.y,150,150);
  ...
  textAlign("center");
  textSize(40);
  text("Mha shooting game", width / 2, 100);
  
 
  textSize(30);
  text("down arrow to aim down",200,700);

  textSize(30);
  text("up arrow to aim up",200,750);

  textSize(30);
  text("space to shoot",200,800);
  ...
  textAlign("center");
  textSize(30);
  text("Remaining Fireballs : " + numberOfArrows, 200, 100);
...
function keyPressed() {
  if (keyCode === 32) {
    if (numberOfArrows > 0) {
      ...
      var arrow = new PlayerArrow(posX + 25, posY - 25, 100, 20, angle);
